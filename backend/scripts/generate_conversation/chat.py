...
class ChatView():
    ...
    def __init__(self, starting_message, user_facts):
        ...
        self.openai_tools = []
        self.USER_INSTRUCTIONS = USER_INSTRUCTIONS_BASE + "\n" + "Facts: " + "\n".join(user_facts)
    ...
    def _reverse_message_roles(self, messages):
        ...
        for message in messages:
            if message["role"] == "user":
                reversed_messages.append({"role": "assistant", "content": message["content"]})
            elif message["role"] == "assistant":
                reversed_messages.append({"role": "user", "content": message["content"]})
            else:
    ...
    def generate_conversation(self, num_turns=5):
        ...
        self.input_messages = self.input_messages[0] # Reset input messages to the first message only
        return chat_history
...
def process_csv(input_file=None, output_file=None, num_turns=5, num_rows=None):
    ...
    if output_file is None:
        output_file = Path(__file__).parent / "tenant_questions_facts_with_new_conversations.csv"
    ...
    df['New Conversation'] = df.apply(lambda row: process_conversation(row, num_turns=num_turns), axis=1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate conversations between a user and a bot.')
    parser.add_argument('--num-turns', type=int, default=5, help='Number of conversation turns')
    parser.add_argument('--num-rows', type=int, default=None, help='Number of rows to process')
