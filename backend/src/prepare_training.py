...
from app import MODEL, SYSTEM_PROMPT
...
def prepare_training_data():
    ...
    if os.path.exists(DATA_FILE):
        ...
        with jsonlines.open(DATA_FILE, mode="r") as inf:
            for item in inf:
                if 'messages' in item and 'metadata' in item and 'session_id' in item['metadata']:
            ...
            for item in inf:
                if 'messages' in item:
                    ...
                    if len(messages) >= 3:  # system + user + assistant
                        ...
                        for idx in user_indices:
                            ...
                            if idx < len(messages) - 1 and messages[idx + 1]["role"] == "assistant":
                                ...
                                window = messages[:idx + 2]  # Include user message and assistant response
                    ...
                    if 'metadata' in item and 'session_id' in item['metadata']:
                        session_id = item['metadata']['session_id']
                        feedback_session_ids.add(session_id)  # Mark this session as coming from feedback
...
if __name__ == "__main__":
    ...
    print(f"openai fine-tuning create -t {output_file} -m {MODEL} --suffix law_chat_with_feedback")